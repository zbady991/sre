...
// import { parseKey, parseTemplate } from '@sre/utils/input.utils';
...
//             let key = input.Key || config.data.key;
//             key = parseTemplate(key, input, { processUnmatched: false });
//             key = await parseKey(key, teamId);
...
//             const Signature = this.signData(data, key, signMethod, encoding, { hashType, RSA_padding, RSA_saltLength });
