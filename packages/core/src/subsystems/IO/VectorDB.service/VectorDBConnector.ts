...
import { IVectorDataSource, QueryOptions, Source, VectorDBMetadata, VectorsResultData } from '@sre/types/VectorDB.types';
...
export interface IVectorDBRequest {
    ...
    insert<T extends Source>(namespace: string, source: IVectorDataSource<T> | IVectorDataSource<T>[]): Promise<void>;
...
export abstract class VectorDBConnector extends SecureConnector {
    ...
    protected abstract insert<T extends Source>(
        ...
        data: {
            indexName: string;
            namespace: string;
            source: IVectorDataSource<T> | IVectorDataSource<T>[];
        }
    ): Promise<void>;
