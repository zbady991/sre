...
function runTestCases(model: string) {
    ...
    it(`runs a simple prompt with Model: ${model}`, async () => {
        const result: any = await llmHelper.promptRequest('Hello, how are you?', config, agent);
        expectValidResponse(result);
    }, 30000);

    it(`runs a prompt with system message with Model: ${model}`, async () => {
    ...
    it(`handles long prompts correctly with Model: ${model}`, async () => {
    ...
    it(`handles complex multi-turn conversations with system message for Model: ${model}`, async () => {
